...
export async function handle(request: Request): Promise<Response> {
  try {
    ...
    if (user.role !== "admin" && subject.teacherId !== effectiveTeacherId) {
      return new Response(
        superjson.stringify({
          error: "You do not have permission to add sections to this subject.",
        }),
        { status: 403 }
      );
    }
    ...
    const newSection = await db
      ...
      .values({
        ...
        maxAttemptsAllowed: input.maxAttemptsAllowed ?? null,
    ...
    console.error("Error creating subject section:", error);
