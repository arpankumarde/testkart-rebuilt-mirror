...
export const schema = z.object({
  ...
  subjectWiseTiming: z.boolean().default(false),
  questionWiseTiming: z.boolean().default(false),
...
export const postTeacherTestItemsCreate = async (
  body: InputType,
  init?: RequestInit
): Promise<OutputType> => {
  const validatedInput = schema.parse(body);
  const result = await fetch(`/_api/teacher/test-items/create`, {
    method: "POST",
