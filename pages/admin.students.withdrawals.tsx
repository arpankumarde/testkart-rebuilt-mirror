...
import styles from "./admin.students.withdrawals.module.css";
...
const PayoutBankDetails = ({ withdrawal }: { withdrawal: AdminStudentWithdrawalRecord }) => {
...
const AdminStudentWithdrawalsPage: React.FC = () => {
  ...
  const handleOpenProcessDialog = (withdrawal: AdminStudentWithdrawalRecord, action: "approve" | "reject") => {
  ...
  const handleProcessSubmit = (values: ProcessFormValues) => {
    ...
    if (actionType === "reject" && !values.notes?.trim()) {
      form.setError("notes", { message: "Give a reason - the student sees it." });
      return;
    }
    ...
    processMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(actionType === "approve" ? "Payout approved." : "Payout rejected. The student has been notified.");
  ...
  const renderIdentity = (withdrawal: AdminStudentWithdrawalRecord, withStatus = false) => {
    const contact = withdrawal.studentEmail || withdrawal.studentMobile || "No email or phone";
    return (
      <div className={styles.stack}>
        <span className={styles.primaryLine}>
          <span className={styles.truncate} title={withdrawal.studentName}>{withdrawal.studentName}</span>
          {withStatus && <StatusFlag status={withdrawal.status} />}
        </span>
        <span className={styles.secondaryLine} title={contact}>{contact}</span>
      </div>
    );
  };

  /* A student's wallet is debited when they ask, so a pending request's
     balance before is the current balance plus the amount. */
  const renderWalletBalance = (withdrawal: AdminStudentWithdrawalRecord) => {
    ...
    const before = formatCurrency(withdrawal.currentWalletBalance + withdrawal.amount);
    const after = formatCurrency(withdrawal.currentWalletBalance);
    return (
      <div className={styles.stack}>
  ...
  const renderNotes = (withdrawal: AdminStudentWithdrawalRecord) => {
  ...
  const renderActions = (withdrawal: AdminStudentWithdrawalRecord) => {
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              aria-label={`Approve payout to ${withdrawal.studentName}`}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              aria-label={`Reject payout to ${withdrawal.studentName}`}
        ...
        <ConsoleListEmpty
          ...
          description={
            isFiltered
              ...
              : "Prize money payout requests from students arrive here for approval."
          }
        >
          {isFiltered && (
            <Button
  ...
  return (
    <>
      <Helmet>
        <title>Student withdrawals - Testkart Admin</title>
        <meta name="description" content="Student payout requests awaiting approval." />
      </Helmet>

      <div className={styles.page}>
        <ConsolePageHeader title="Student withdrawals" />
        ...
        <ConsoleListToolbar
          ...
          search={{
            ...
            label: "Search student withdrawals",
          }}
        />
      ...
      >
        <ConsoleDialogContent size="md">
          <ConsoleDialogHeader
            ...
            description={
              actionType === "reject"
                ? "This request will not be paid. Say why below - the student sees it."
                ...
                <>
                  <section className={styles.dialogBlock}>
                    ...
                    <dl className={styles.infoGrid}>
                      <div className={styles.infoItem}>
                        <dt>Student</dt>
                        <dd>{selectedWithdrawal.studentName}</dd>
                      </div>
                      <div className={styles.infoItem}>
                        <dt>Amount</dt>
...
export default AdminStudentWithdrawalsPage;
