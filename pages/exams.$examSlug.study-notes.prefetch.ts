...
// the queryKey/filters used by StudyNotesBody in ExamProductListingPage.tsx
// (note: examId/page/limit are numbers here, not strings, matching that
// hook's filters object exactly).
export const prefetch: PagePrefetchFn = async (ctx) => {
  ...
  try {
    ...
    await qc.prefetchQuery({
      ...
      queryFn: () => fetchShopProductsListServer(filters),
    });
    ...
    console.error("Error prefetching exam study notes:", error);
  }
