...
const rejectFormSchema = z.object({
  rejectionReason: z.string().min(10, "Give a reason of at least 10 characters so the student knows what to fix."),
});
type RejectFormValues = z.infer<typeof rejectFormSchema>;

const AdminStudentBankDetailsPage: React.FC = () => {
  ...
  const handleVerify = () => {
    ...
    verifyMutation.mutate(
      { bankDetailsId: selectedDetail.id, action: 'verify' },
      {
        onSuccess: () => {
  ...
  const handleReject = (values: RejectFormValues) => {
    if (!selectedDetail) return;
    verifyMutation.mutate(
      { bankDetailsId: selectedDetail.id, action: 'reject', rejectionReason: values.rejectionReason },
      {
        onSuccess: () => {
          toast.success("Bank details rejected. The student has been notified.");
          closeDialog();
        },
        onError: (err) => {
  ...
  const renderStatusFlag = (detail: StudentBankDetailsAdminView) => (
  ...
  const renderPanThumb = (detail: StudentBankDetailsAdminView) =>
    detail.panCardImageBase64 ? (
      <button
        type="button"
        className={styles.panThumbButton}
        onClick={() => openDialog('view', detail)}
        aria-label={`View PAN card for ${detail.studentName}`}
      >
  ...
  const renderActions = (detail: StudentBankDetailsAdminView) => (
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            aria-label={`View bank details for ${detail.studentName}`}
            onClick={() => openDialog('view', detail)}
          >
            <Eye />
          </Button>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              size="icon-md"
              className={styles.iconButton}
              aria-label={`Verify bank details for ${detail.studentName}`}
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              aria-label={`Reject bank details for ${detail.studentName}`}
        ...
        <ConsoleListEmpty
          ...
          description={
            isFiltered
              ...
              : "Students who add payout details for prize money appear here."
          }
        >
          {isFiltered && (
            ...
            <tbody>
              {data.bankDetails.map((detail) => (
                <tr key={detail.id}>
                  <td>
                    <div className={styles.stack}>
                      <span className={styles.primaryLine} title={detail.studentName}>{detail.studentName}</span>
                      <span className={styles.secondaryLine} title={detail.studentEmail}>{detail.studentEmail}</span>
                    </div>
                  </td>
                  <td>
                    <div className={styles.stack}>
              ...
              <div className={styles.cardHeader}>
                <div className={styles.stack}>
                  <span className={styles.cardTitleLine}>
                    <span className={styles.truncate} title={detail.studentName}>{detail.studentName}</span>
                    {renderStatusFlag(detail)}
                  </span>
                  <span className={styles.secondaryLine} title={detail.studentEmail}>{detail.studentEmail}</span>
                </div>
  ...
  return (
    <>
      <Helmet>
        <title>Student bank details - Testkart Admin</title>
        <meta name="description" content="Student payout details awaiting verification." />
      </Helmet>
      <div className={styles.page}>
        <ConsolePageHeader title="Student bank details" />
        ...
        <ConsoleListToolbar
          ...
          search={{
            ...
            label: "Search student bank details",
          }}
        />
      ...
      <Dialog open={dialogState === 'view' && !!selectedDetail} onOpenChange={(open) => !open && closeDialog()}>
        <ConsoleDialogContent size="xl">
          <ConsoleDialogHeader
            title={`Bank details for ${selectedDetail?.studentName ?? ""}`}
            description={selectedDetail?.studentEmail}
          >
            {selectedDetail && (
                ...
                <section className={styles.dialogBlock}>
                  ...
                  {selectedDetail.panCardImageBase64 ? (
                    <img
                      ...
                      alt={`PAN card for ${selectedDetail.studentName}`}
      ...
      <ConsoleConfirmDialog
        ...
        description={
          selectedDetail ? (
            <>Payouts to <strong>{selectedDetail.studentName}</strong> will be enabled once verified.</>
          ) : undefined
        }
      ...
      <Dialog open={dialogState === 'reject' && !!selectedDetail} onOpenChange={(open) => !open && closeDialog()}>
        <ConsoleDialogContent size="sm">
          <ConsoleDialogHeader
            ...
            description={
              <>Say what is wrong so <strong>{selectedDetail?.studentName}</strong> can fix it. They will see this reason.</>
            }
...
export default AdminStudentBankDetailsPage;
