...
>(({ className, onBlur: onBlurProp, ...props }, ref) => {
  const { formItemId, error, formMessageId, onBlur } = useFormField();
  
  const handleBlur = (e: React.FocusEvent<HTMLElement>) => {
    // Call the original onBlur if provided
    onBlurProp?.(e);
    // Defer validation until after React state updates complete
    // This allows components like Radix Select to update their values before validation runs
    setTimeout(() => onBlur(), 0);
  };
