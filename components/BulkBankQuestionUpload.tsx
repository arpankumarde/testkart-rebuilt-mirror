...
import { useBulkUploadBankQuestionsMutation } from '../helpers/useTeacherQuestionBank';
import styles from './BulkBankQuestionUpload.module.css';
...
interface BulkBankQuestionUploadProps {
  className?: string;
  examId?: number;
  subjectName?: string;
...
export const BulkBankQuestionUpload: React.FC<BulkBankQuestionUploadProps> = ({
  className,
  examId,
  subjectName,
  onSuccess,
  ...
  const bulkUploadMutation = useBulkUploadBankQuestionsMutation();
  ...
  const handleDownloadTemplate = () => {
    ...
    link.download = 'question-bank-upload-template.csv';
  ...
  const processFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log(`[BulkBankUpload] File: ${file.name}, Size: ${data.length} bytes, Type: ${file.type}`);
        const workbook = XLSX.read(data, { type: 'array' });
        const sheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[sheetName];
        const json = XLSX.utils.sheet_to_json(worksheet, { defval: "", raw: false });
        console.log(`[BulkBankUpload] Sheets: ${workbook.SheetNames.join(', ')}, Rows parsed: ${json.length}`);
        if (json.length > 0) {
          console.log(`[BulkBankUpload] Headers detected: ${Object.keys(json[0] as any).join(', ')}`);
        }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success(`${result.count} questions imported to bank successfully!`);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className={styles.dialogContent}>
        <DialogHeader>
          <DialogTitle>Bulk Upload to Question Bank</DialogTitle>
          <DialogDescription>
            {step === 'upload'
